...
`validate`/`plan`/`templates` work from anywhere (`templates` takes --server URL, default
$GNS3_SERVER or http://localhost). `docker` and `qemu` touch the local docker daemon and
filesystem, so they run **on the GNS3 VM** — the Ansible wrapper syncs this tree there and
invokes them over SSH.
...
def cmd_validate(args):
    ...
    # A Qemu template must actually point at the disk image the manifest installs,
    # otherwise the node fails to start with a "image not found" long after the build.
    ...
    for key, node in m["nodes"].items():
        ...
        for name in node.get("templates", []):
            ...
            if node["file"] not in [t.get(f) for f in image_fields]:
                problems.append(f"qemu node '{key}': template '{name}' does not reference "
                                f"{node['file']} (has {t.get('hda_disk_image')!r})")
...
def qemu_install(node, images_dir, force, verify, dry_run):
    """Ensure one Qemu image is present and verified. Returns 'installed'|'skipped'."""
...
def main():
    ...
    return {"validate": cmd_validate, "plan": cmd_plan, "templates": cmd_templates,
            "docker": cmd_docker, "qemu": cmd_qemu}[args.cmd](args)
